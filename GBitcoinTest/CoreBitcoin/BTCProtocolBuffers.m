...
+ (NSData *) lenghtDelimitedDataAtOffsets:(NSInteger *)offset fromData:(NSData*)src {
...
+ (NSInteger) fieldAtOffset:(NSInteger *)offset int:(uint64_t *)i fixed32:(uint32_t *)fixed32 fixed64:(uint64_t *)fixed64 data:(NSData **)d fromData:(NSData*)src {
    ...
    switch (key & 0x07) {
        ...
        case BTCProtobufTypeLengthDelimited: {
            lengthDelimitedData = [self lenghtDelimitedDataAtOffsets:offset fromData:src];
...
+ (void) writeLengthDelimitedDatas:(NSData*)data toData:(NSMutableData*)dst {
...
+ (void) writeData:(NSData*)data withKey:(NSInteger)key toData:(NSMutableData*)dst {
    ...
    [self writeLengthDelimitedDatas:data toData:dst];
}
